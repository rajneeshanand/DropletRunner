#!/usr/bin/env python3
"""
State estimation node for DropletRunner.
Uses OCamCalib camera model for lens distortion correction.
Detects droplet position, corner markers, estimates board tilt,
and converts to metric coordinates via distortion-corrected homography.

KALMAN FILTER: When the droplet is not detected (e.g. specular glare zone),
the filter predicts position from last known state + velocity. This fills
gaps of up to ~3 seconds (MAX_PREDICT_FRAMES) so the RL agent always
receives a position estimate.
"""
...
class StateEstimator(Node):
    def __init__(self):
        ...
        # Board ROI in pixels (tuned for your setup)
        self.roi_x1, self.roi_y1 = 338, 126
        self.roi_x2, self.roi_y2 = 908, 648

        # Physical board dimensions (mm)
        ...
        # Measurement matrix: we observe [x_mm, y_mm]
        self.kf.measurementMatrix = np.array([
            [1, 0, 0, 0],
            [0, 1, 0, 0],
        ], dtype=np.float32)

        # Process noise — allow for acceleration/direction changes
        ...
        # Measurement noise — detection jitter ~2mm
        ...
        # Publish state: [x_mm, y_mm, alpha, beta, detected]
    ...
    def _undistort_corners(self, corners_px):
        """
        Undistort corner pixel coordinates using OCamCalib model.
        """
        undistorted = []
    ...
    def _compute_homography(self, corners_px):
        """
        Compute homography from undistorted corner positions to mm.
        """
    ...
    def _pixel_to_mm(self, px, py):
        """
        Convert pixel coordinates to mm using OCamCalib undistortion
        + homography.
        """
    ...
    def _estimate_tilt_from_corners(self, corners_px):
        """
        Estimate board tilt angles from current corner positions
        compared to reference positions.
        """
    ...
    def image_callback(self, msg):
        ...
        # ── Detect droplet in board region ─────────────────────

...
Uses trained DreamerV3 policy with exploration noise to collect episodes.
Falls back to random policy if no checkpoint is found.

Following Thomas Bi's CyberRunner approach:
- Uses agent.policy(mode='explore') which adds noise for exploration
- Saves episodes as .npz for offline retraining on HPC

HOLD-ACTION LOGIC: When the estimator reports detected=0.5 (Kalman prediction)
for more than HOLD_THRESHOLD consecutive frames, the data collector freezes
the motor command to the last good action. This prevents the policy from
reacting to drifting Kalman predictions during long glare-zone dropouts.
Short dropouts (1-5 frames) still use the policy normally.
...
class DataCollector(Node):
    ...
    def control_step(self):
        ...
        # ── Path tracker update ────────────────────────────────
        if detected == 1.0:
            # Real detection — use actual position
            ...
            # Long Kalman prediction — use last good position
            # to avoid path tracker seeing drifted predictions
        ...
        # ── Build observation vector (14-dim) ──────────────────
    ...
    def _learned_policy(self, patch, obs_vector, reward):
        """
        Use trained DreamerV3 policy with exploration noise.
        mode='explore' adds noise like Thomas's approach.
        """
        obs = {
            'image': patch[None],

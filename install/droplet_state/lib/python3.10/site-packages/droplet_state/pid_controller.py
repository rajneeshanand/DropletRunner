#!/usr/bin/env python3
"""
pid_controller.py — PID baseline controller for DropletRunner.

Baseline controller for comparison against MBRL (DreamerV3) policy.
Uses the SAME PathTracker as policy_runner.py for identical progress
computation, termination, and success criteria.

Logs per-step CSV data in the same format as policy_runner.py for
direct comparison plots.

Usage:
    1. Start camera + estimator + motor_driver as usual
    2. Run this instead of policy_runner.py:
       ros2 run droplet_state pid_controller
    3. Place droplet at start, press Enter
    4. Results saved to:
       - ~/droplet_runner_ws/pid_logs/pid_actions_ep{timestamp}.csv  (per-step)
       - ~/droplet_runner_ws/pid_results.json  (summary)

Motor mapping (from your hardware):
    Motor 1 (action[0]) → controls Y-axis tilt
    Motor 2 (action[1]) → controls X-axis tilt
    Signs may need flipping — if droplet moves AWAY from target,
    flip sign_m1 or sign_m2.
"""
...
class PIDController(Node):
    def __init__(self):
        ...
        # Anti-windup: limit the integral accumulator
        self.max_integral = 25.0

        # Motor sign mapping: +1 or -1
        self.sign_m1 = 1.0    # Sign for Motor 1 (Y-axis)
        self.sign_m2 = -1.0   # Sign for Motor 2 (X-axis)

        # Max motor command (same as MBRL: ±150)
        self.max_action = 150.0

        # Waypoint advance threshold (mm)
        self.waypoint_threshold = 8.0

        # Episode termination
        self.max_steps = 600   # Max steps per episode (at 20Hz = 30 seconds)
        ...
        # PathTracker for progress (SAME as MBRL policy_runner)
        possible_paths = [
        ...
        # Action logging (same format as policy_runner)
        ...
        # Input thread for episode management
        import threading
        self.input_thread = threading.Thread(target=self.wait_for_input, daemon=True)
        self.input_thread.start()

    def load_waypoints(self):
        """Load waypoints from the same JSON file used by path_tracker."""
        possible_paths = [
            os.path.expanduser('~/droplet_runner_ws/src/droplet_state/config/path_waypoints_mm.json'),
    ...
    def state_callback(self, msg):
        """Receive droplet state from estimator."""
        self.current_state = np.array(msg.data)

    def wait_for_input(self):
        """Wait for user to press Enter to start each episode."""
        while rclpy.ok():
    ...
    def start_episode(self):
        ...
        # Reset PathTracker (same as MBRL)
        self.path_tracker.reset()

        # Open CSV log file (same format as policy_runner)
    ...
    def control_loop(self):
        """Main PID control loop — runs at 20Hz."""
        if not self.running or self.current_state is None:
            return

        self.step_count += 1

        # Extract current state (same fields as policy_runner)
        ...
        # Compute progress using PathTracker (SAME as MBRL)
        if detected > 0.5:
        ...
        # Check termination via PathTracker (SAME as MBRL)
        ...
        # PID: Motor 1 controls error[0], Motor 2 controls error[1]
        ...
        # Log every step to CSV (same format as policy_runner)
        if self.action_log_file is not None:

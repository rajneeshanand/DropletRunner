#!/usr/bin/env python3
"""
Open-Loop Replay: Plays back recorded motor commands from a successful
closed-loop episode WITHOUT using camera feedback to choose actions.

This demonstrates that the RL agent's success depends on closed-loop
feedback — replaying the same actions open-loop will fail because
small errors accumulate without correction.

Usage:
    ros2 run droplet_state open_loop_replay --ros-args \
        -p csv_file:=/path/to/mbrl_actions_epXXXXXX.csv

The camera + estimator still run (to record where the droplet actually goes),
but the motor commands are entirely pre-recorded — the agent makes NO decisions.
"""
...
class OpenLoopReplay(Node):
    def __init__(self):
        ...
        # Motor command publisher (same topic as policy_runner uses)
        self.motor_pub = self.create_publisher(Float32MultiArray, '/motor_commands', 10)

        # Subscribe to estimator for recording actual position
        ...
        # Control rate: 20 Hz (same as closed-loop)
        self.dt = 0.05
        self.timer = None
    ...
    def state_callback(self, msg):
        """Record actual droplet position from estimator."""
        if len(msg.data) >= 2:
            self.current_pos = (msg.data[0], msg.data[1])

    def wait_for_start(self):
        """Wait for user to press ENTER, then start replay."""
        input()
    ...
    def replay_step(self):
        """Send the next pre-recorded motor command."""
        if not self.running:
            return
        ...
        # Publish motor command (same format as policy_runner)
